...
class TrainerNNE(BaseTrainer):
    ...
    def __init__(self, model, criterion, 
                 metric_ftns, optimizer, 
                 config, device, 
                 data_loader, lr_scheduler=None, 
                 len_epoch=None):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, instance in enumerate(self.train_data_loader):
            ...
            # Metric visualize training and validation step
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, instance in enumerate(self.train_data_loader):
            ...
            input_ids = torch.tensor(instance['input_ids']).to(self.device)
            attention_mask = torch.tensor(instance['attention_mask']).to(self.device)
            flat_lm_conll_ids = torch.tensor(instance['flat_lm_conll_ids']).to(self.device)


            self.optimizer.zero_grad()
            output = self.model(input_ids, attention_mask)
            loss = self.criterion(output, flat_lm_conll_ids)
            loss.backward()
            self.optimizer.step()
